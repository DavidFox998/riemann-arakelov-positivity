...
theorem abbes_ullmo_1996_1_2 (N : ℕ) (hg : (2 : ℚ) ≤ (X₀ N).genus) :
  ...
  apply div_pos; · have : (0 : ℚ) < (X₀ N).genus - 1 := by linarith
...
private theorem sqrt13_lt_4 : Real.sqrt 13 < 4 := by
  have h16 : (4 : ℝ) = Real.sqrt 16 := by
    rw [show (16 : ℝ) = 4 ^ 2 from by norm_num]; exact (Real.sqrt_sq (by norm_num)).symm
  rw [h16]; exact Real.sqrt_lt_sqrt (by norm_num) (by norm_num)

theorem C_S14_143_gt_tau : C_S14_143 > 2 * Real.sqrt 13 := by
  unfold C_S14_143; have h8 : (8 : ℝ) < 862925199 / 100000000 := by norm_num
...
theorem bost_connes_threshold : 2 * Real.sqrt ((X₀ 143).genus : ℝ) < 320 := by
  have hg : ((X₀ 143).genus : ℝ) = 13 := by exact_mod_cast X₀_143_genus; rw [hg]; linarith [sqrt13_lt_4]
...
theorem log_11_gt_one : (1 : ℝ) < Real.log 11 := by
  ...
  have h_log := Real.log_lt_log (Real.exp_pos 1) h_exp; rwa [Real.log_exp] at h_log
...
theorem arakelovPairing_X0_143_pos : (0 : ℝ) < arakelovPairing_X0_143 := by
  have h11 : (1 : ℝ) < Real.log 11 := log_11_gt_one; have h13 : (0 : ℝ) < Real.log 13 := Real.log_pos (by norm_num)
  unfold arakelovPairing_X0_143 K_143_val K_infty_143
  have hlog : (24 : ℝ) * Real.log 143 = 24 * Real.log 11 + 24 * Real.log 13 := by rw [log_143_eq_log_11_add_log_13]; ring
  linarith
...
-- §4-§5. Definitions for conditional part — no sorry
...
def ExplicitFormula_ZeroSum (S_weil : ℝ → ℂ) : Prop :=
  ∀ t T, 1 < T → L_fn t = 0 → t ≠ 0 → t ≠ 1/2 → ‖(Real.sqrt T : ℂ)‖ ≤ ‖S_weil T * (t * Real.log T) / (↑T : ℂ) ^ (t : ℂ)‖

def ZeroOffCriticalLine_Contradiction (S_weil : ℝ → ℂ) : Prop :=
  ∀ t, L_fn t = 0 → t ≠ 0 → t ≠ 1/2 → ∃ T₀, 1 < T₀ ∧ C_S14_143 * T₀ / Real.log T₀ < ‖S_weil T₀‖
...
theorem BC6_direct_CLOSED :
  ...
  intro _ _ T hT; simp; have : 0 < Real.log T := Real.log_pos hT
  have : 0 < C_S14_143 := by linarith [Real.sqrt_nonneg 13, C_S14_143_gt_tau]; positivity
...
theorem Langlands_Descent_CLOSED
  ...
  intro ρ hzero h_one h_triv; unfold L_fn_complex L_143a1 at hzero
  have h1 : (5759 : ℂ) / 10000 ≠ 0 := by norm_num
  have h2 : ρ - 1 = 0 := by by_contra h; have := mul_ne_zero h1 h; contradiction
  have : ρ = 1 := sub_eq_zero.mp h2; contradiction
...
-- Conditional — 0 sorry, hypotheses as args (like your Sarnak file)
