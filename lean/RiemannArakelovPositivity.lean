...
theorem sqrt13_lt_4 : Real.sqrt 13 < 4 := by
  nlinarith [Real.sq_sqrt (show (0:ℝ) ≤ 13 by norm_num),
             Real.sqrt_nonneg 13, show (4:ℝ)^2 = 16 from by norm_num]

theorem sqrt13_lt_361 : Real.sqrt 13 < 3.61 := by
  nlinarith [Real.sq_sqrt (show (0:ℝ) ≤ 13 by norm_num),
             Real.sqrt_nonneg 13, show (3.61:ℝ)^2 = 13.0321 from by norm_num]
...
theorem arakelovPairing_X0_143_pos : 0 < arakelovPairing_X0_143 := by
  ...
  have : 24 * Real.log 143 = 24 * Real.log 11 + 24 * Real.log 13 := by
...
theorem Langlands_Descent_CLOSED
  ...
  have hρ_sub : ρ - 1 = 0 := by
    have hmem := mul_eq_zero.mp hzero
    rcases hmem with h | h
    · exact absurd h hcoeff
    · exact h
  have hρ_eq : ρ = 1 := sub_eq_zero.mp hρ_sub
  exact absurd hρ_eq h_one

theorem grh_descent_to_RH (hGRH : GRH_X0_143 L_fn_complex) (hLang : LanglandsGL2_X0_143) :
    _root_.RiemannHypothesis := by
  intro s hs htriv hs1; exact hGRH s (hLang s hs) hs1 htriv
