/-
  ...
  ### Part 2 — OPEN surface: `M4_ExceptionalWeilBridge`
  ...
  `M4_ExceptionalWeilBridge` names this surface exactly so future work has
  ...
  `M4_ExceptionalWeilBridge` as the single remaining open input.
  NOT a brick.
  ...
  * `M4_ExceptionalWeilBridge` is vacuously satisfiable (if we set
...
/-- **M4 Binding — Arakelov positivity of X₀(143). (BRICK)**
    ...
    `M4_ExceptionalWeilBridge` (the M9 Weil-transfer step).
...
/-- **M4 Exceptional-Set / Weil-Bridge surface (OPEN).**
    ...
    NOT a brick. -/
def M4_ExceptionalWeilBridge : Prop :=
  ArakelovPositivity (X₀ 143) → _root_.RiemannHypothesis
...
/-- **C08 conditional combinator (NOT a brick).**
    ...
    `M4_ExceptionalWeilBridge`.

    NOT a brick.  SORRY: 0.  RH: OPEN. -/
theorem C08_RH_of_M4WeilBridge
    (hbridge : M4_ExceptionalWeilBridge) :
