/-
  ...
  once two named open surfaces are proved:
    - `LambdaToNu`:  λ₁(Y₀(N)) = 1/4 − ν(N)²  (Selberg spectral theory)
    - `NuBound`:     squarefree N ⟹ |ν(N)| ≤ 7/64  (Kim-Sarnak 2003)
  ...
  until BOTH `LambdaToNu` and `NuBound` are formalised.
...
noncomputable def spectral_parameter_mt : ℕ → ℝ
...
/-- **kim_sarnak_squarefree_scaffold — kim_sarnak_squarefree as a conditional theorem.**
    ...
    Given:
      h_ltn : LambdaToNu_mt    (Selberg: λ₁ = 1/4 − ν²)
      h_nu  : NuBound_mt       (Kim-Sarnak: squarefree N ⟹ |ν| ≤ 7/64)
    ...
    When Mathlib has:
      - Selberg spectral theory for Γ₀(N) (to prove LambdaToNu_mt)
      - Gelbart-Jacquet + Kim-Shahidi + squarefree lemma (to prove NuBound_mt)
