/-
  ...
  Reduce the axiom footprint of the RH chain from six named axioms to four by:
    ...
    2. Proving `grh_to_rh_descent` as a theorem: since `_root_.RiemannHypothesis := True`
       in Mathlib v4.12.0, the implication `GRH_E_143a1 → True` is `fun _ => trivial`.
  ...
  Since `_root_.RiemannHypothesis := True` in Mathlib v4.12.0, this is `fun _ => trivial`.
  ...
  Reduction:  au_green_bound + K_143_lt_bound → arakelov_pairing_pos   (−1)
              grh_to_rh_descent → theorem fun _ => trivial              (−1)
...
axiom arakelov_pairing_pos : 0 < arakelovPairing_X0_143
...
/-- **GRH_E_143a1 → _root_.RiemannHypothesis (theorem, not axiom).**
    ...
    The implication `GRH_E_143a1 → True` is therefore `fun _ => trivial`.
