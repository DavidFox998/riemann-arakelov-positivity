...
theorem exp_loglog_dominates_sq (C c₁ : ℝ) (hC : 0 < C) (hc₁ : 0 < c₁) :
  ...
  rw [show (fun t : ℝ => Real.exp (c₁ * Real.log t / Real.log (Real.log t))) =
        (fun x => Real.exp x) ∘ (fun t => c₁ * Real.log t / Real.log (Real.log t)) by rfl]
  have hlog_log : Tendsto (fun t : ℝ => Real.log (Real.log t)) atTop atTop := by
  ...
  have hratio : Tendsto (fun t : ℝ => c₁ * Real.log t / Real.log (Real.log t)) atTop atTop := by
    have hll_pos : ∀ᶠ t in atTop, (0 : ℝ) < Real.log (Real.log t) := by
    ...
    have hdiv : Tendsto (fun t : ℝ => Real.log t / Real.log (Real.log t)) atTop atTop := by
      exact Tendsto.div_atTop hlog_log hlog
        (fun t ht => (Real.log_pos (Real.log_pos ht)).ne')
    simpa using Tendsto.mul_const (c := c₁) hdiv
  ...
  have hlog2 : Tendsto (fun t : ℝ => (Real.log t) ^ 2) atTop atTop := by
    exact hlog.pow_atTop (by norm_num : (0 : ℕ) < 2)
  have hdiv2 : Tendsto (fun t : ℝ =>
  ...
  have : ∀ᶠ t in atTop, C < Real.exp (c₁ * Real.log t / Real.log (Real.log t)) / (Real.log t) ^ 2 := by
    filter_upwards [hdiv2] with t ht
    exact lt_of_lt_of_le (by linarith [show (0 : ℝ) < 1 from by norm_num]) ht
...
theorem riemannHypothesis_of_growth_and_repulsion
  ...
  have hdom := eventually_atTop.1 (exp_loglog_dominates_sq C c₁ hC_pos hc₁_pos)
  obtain ⟨T, hT⟩ := hdom
  obtain ⟨t, ht_B, ht_large⟩ := hlarge T
  have ht_ge_T : T ≤ t := ht_B
  have hgt : C * (Real.log t) ^ 2 <
    Real.exp (c₁ * Real.log t / Real.log (Real.log t)) := hT t ht_ge_T
  have ht_ge_2 : 2 ≤ t := by
    by_contra h
    push_neg at h
    have : t < 2 := h
    have : Real.log t < Real.log 2 := Real.log_lt_log (by linarith) this
    linarith [Real.log_pos (show (0 : ℝ) < 1 from by norm_num),
              show (0 : ℝ) < Real.log 2 from Real.log_pos (by norm_num)]
  have hbound_t : ‖riemannZeta (1/2 + (t : ℂ) * I)‖ ≤ C * (Real.log t) ^ 2 :=
    hbound t ht_ge_2
  have hlog_t_pos : (0 : ℝ) < Real.log t := Real.log_pos (by linarith)
  have hlog_log_pos : (0 : ℝ) < Real.log (Real.log t) := Real.log_pos hlog_t_pos
