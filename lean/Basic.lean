...
theorem L_143a1_deriv_at_one : deriv L_143a1 1 = (5759 : ℂ) / 10000 := by
  unfold L_143a1
  rw [deriv_mul_const, deriv_sub_const]
  · rfl
  · simp
...
/-- **exp_loglog_dominates_sq** (PROVED, 0 sorry):
    ...
    Proof uses Real.tendsto_exp_div_pow_atTop. -/
theorem exp_loglog_dominates_sq (C c₁ : ℝ) (hC : 0 < C) (hc₁ : 0 < c₁) :
    ∀ᶠ t in atTop,
      C * (Real.log t) ^ 2 < Real.exp (c₁ * Real.log t / Real.log (Real.log t)) := by
  have key : Tendsto (fun x : ℝ => Real.exp x / x ^ 2) atTop atTop :=
    tendsto_exp_div_pow_atTop 2
  have hlog_log_tendsto : Tendsto (fun t : ℝ => Real.log (Real.log t)) atTop atTop := by
    apply Tendsto.comp Real.tendsto_log_atTop
    filter_upwards [eventually_ge_atTop (Real.exp 1 : ℝ)] with t ht using Real.log_pos ht
  have hratio : Tendsto (fun t : ℝ => c₁ * Real.log t / Real.log (Real.log t)) atTop atTop := by
    ...
    have : Tendsto (fun t : ℝ => Real.log t / Real.log (Real.log t)) atTop atTop := by
      apply Tendsto.congr' (g := fun t => (Real.log t) / Real.log (Real.log t))
      · filter_upwards with t using fun t => by rfl
      · exact Tendsto.div_atTop hlog_log_tendsto (Real.tendsto_log_atTop)
            (fun t ht => Real.log_pos (Real.log_pos ht) |>.ne')
    simp only [← this]
    exact tendsto_const_mul_atTop_of_pos (fun t ht => hc₁.le)
  have hexp : Tendsto (fun t : ℝ => Real.exp (c₁ * Real.log t / Real.log (Real.log t))) atTop atTop := by
    exact Tendsto.comp Real.tendsto_exp_atTop hratio
  have hlog2 : Tendsto (fun t : ℝ => (Real.log t) ^ 2) atTop atTop := by
    exact (Real.tendsto_log_atTop).pow_atTop (by norm_num)
  have hdiv : Tendsto (fun t : ℝ =>
      Real.exp (c₁ * Real.log t / Real.log (Real.log t)) / (Real.log t) ^ 2) atTop atTop := by
    exact Tendsto.div_atTop hlog2 hexp (fun t ht => pow_pos (Real.log_pos ht) 2 |>.ne')
...
/-- **Route A conditional**: GrowthBound + ZeroRepulsion → RH.
    ...
    This theorem is a named conditional — not a proof claim. -/
theorem riemannHypothesis_of_growth_and_repulsion
    (hG : GrowthBound) (hR : ZeroRepulsion) :
    _root_.RiemannHypothesis := by
  intro s hs htriv hs1
  by_contra hcontra
  -- If s is a nontrivial zero with Re(s) ≠ 1/2, ZeroRepulsion gives large values
  have hoff : ∃ ρ : ℂ, riemannZeta ρ = 0 ∧
    (¬ ∃ n : ℕ, ρ = -2 * ((n : ℂ) + 1)) ∧ ρ ≠ 1 ∧ ρ.re ≠ 1 / 2 :=
    ⟨s, hs, htriv, hs1, hcontra⟩
  obtain ⟨c₁, hc₁_pos, hlarge⟩ := hR hoff
  obtain ⟨C, hC_pos, hbound⟩ := hG
  -- The growth bound says |ζ(1/2+it)| ≤ C·(log t)²
  -- ZeroRepulsion says |ζ(1/2+it)| ≥ exp(c₁·log t / log log t) for arbitrarily large t
  -- But exp(c₁·log t / log log t) eventually exceeds C·(log t)² (proved above)
  -- This is a contradiction
  obtain ⟨t, ht_B, ht_large⟩ := hlarge (C * (2 : ℝ) ^ 2)
  have ht_ge_2 : 2 ≤ t := by
    linarith [show (0 : ℝ) ≤ C * 4 from mul_nonneg hC_pos.le (by norm_num),
              show (0 : ℝ) < 4 * 1 from by norm_num]
  ...
  have ht_T : T ≤ t := by linarith [show (0 : ℝ) ≤ C * 4 from le_of_lt hC_pos]
  have hgt : C * (Real.log t) ^ 2 < Real.exp (c₁ * Real.log t / Real.log (Real.log t)) :=
    hT t (by linarith)
  have hlog_log_pos : (0 : ℝ) < Real.log (Real.log t) :=
    Real.log_pos hlog_t_pos
  linarith [hbound_t, ht_large hlog_log_pos.le]
